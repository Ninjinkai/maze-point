...
static func _build_candidate(width: int, height: int, rng: RandomNumberGenerator, difficulty_scale: float) -> Dictionary:
	...
	for desired_moves in move_options:
		...
		if intended_path.is_empty():
			continue
		if not MetricsScript.path_has_shape_variety(intended_path, start, goal):
			continue
...
static func _pick_start_and_goal(width: int, height: int, rng: RandomNumberGenerator) -> Dictionary:
	...
	for _attempt in range(64):
		...
		var score: float = MetricsScript.score_endpoint_pair(start, goal)
		if score > best_score:
			best_score = score
			best_start = start
			best_goal = goal
...
static func _extend_path(width: int, height: int, goal: Vector2i, desired_moves: int, rng: RandomNumberGenerator, path: Array[Vector2i], visited: Dictionary) -> bool:
	...
	neighbors.sort_custom(func(a: Vector2i, b: Vector2i) -> bool:
		var a_score: float = MetricsScript.score_path_neighbor(a, current, goal, previous_direction, remaining_moves, width, height, orthogonal_neighbors(width, height, a).size())
		var b_score: float = MetricsScript.score_path_neighbor(b, current, goal, previous_direction, remaining_moves, width, height, orthogonal_neighbors(width, height, b).size())
